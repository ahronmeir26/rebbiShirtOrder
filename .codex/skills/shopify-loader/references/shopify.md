...
- Dashboard cancellation uses Admin GraphQL `orderCancel` with `refundMethod.originalPaymentMethodsRefund: false`, then attempts `orderClose` (`OrderCloseInput.id`) to archive/close the canceled order. If closing fails after cancellation succeeds, preserve the cancellation and save the close error on `shopifyCancellation.closeError`.
